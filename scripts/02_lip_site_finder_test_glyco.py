...
#db_path = r"D:\Manuscripts\2024_Haiyan_VitaminA\UniProt_PRIDE_retrival\uniprot-download_true_format_fasta_query__28Homo_20sapiens_29_20AND_-2022.11.22-18.39.02.76.fasta" #database fasta path
...
data_path = r"D:\Manuscripts\2024_Haiyan_VitaminA\glyco_Uniprot_test\Conformotypic peptides match_Lung_glyco_short.csv"
output_path = r"D:\Manuscripts\2024_Haiyan_VitaminA\glyco_Uniprot_test"
PTM_site_variance = 10
...
def UniProt_search(prot_query):
   ...
   """
    ...
    # Extract the 'Modified residue' value
    ptm = res['Modified residue'].iloc[0]
    glyco = res['Glycosylation'].iloc[0]
    
    ptm_df = pd.DataFrame()

    if not (isinstance(glyco, float) and np.isnan(glyco)):
        glyco_split = glyco.split('CARBOHYD ')[1:] # Split the PTM string by 'MOD_RES '
        glyco_df = pd.DataFrame({'Res #': glyco_split})
        ptm_df = pd.concat([ptm_df,glyco_df],ignore_index=True)
    else:
        pass

    if not (isinstance(ptm, float) and np.isnan(ptm)):
        ptm_split = ptm.split('MOD_RES ')[1:] # Split the PTM string by 'MOD_RES '
        ptm_split_df = pd.DataFrame({'Res #': ptm_split})
        ptm_df = pd.concat([ptm_df,ptm_split_df],ignore_index=True)
    else:
        pass

    # Check if 'ptm' is NaN
    if len(ptm_df) == 0:
        # No modified residues found; return an empty DataFrame
        return pd.DataFrame(columns=['Res #', 'type', 'evidence'])

    # Split into three columns: Res #, type, and evidence placeholder
    # Example format: "R11413-3:26; /note="Phosphoserine""
    ptm_df[['Res #', 'type', 'evidence']] = ptm_df['Res #'].str.split('; /', expand=True)
    
    # Clean up the 'type' column by removing note= and quotes
    ptm_df['type'] = ptm_df['type'].str.replace(r'^note="', '', regex=True)
    ptm_df['type'] = ptm_df['type'].str.replace(r'"$', '', regex=True)
    
    # Assign a fixed evidence
    ptm_df['evidence'] = 'UniProt'
    
    # Extract isoform and residue number from Res #
    # Pattern assumes format like "R11413-3:26"
    isoform_pattern = re.compile(r'^(?P<isoform_info>[^:]+):(?P<res_num>\d+)$')
    matches = ptm_df['Res #'].str.extract(isoform_pattern)
    
    # If the match is successful, replace columns accordingly
    ptm_df['isoform_info'] = matches['isoform_info']
    ptm_df['Res #'] = matches['res_num'].where(matches['res_num'].notna(), ptm_df['Res #'])
    
    # Convert Res # to int if possible
    ptm_df['Res #'] = pd.to_numeric(ptm_df['Res #'], errors='ignore')
    
    # Incorporate isoform info directly into the type description
    ptm_df['type'] = ptm_df.apply(
        lambda x: f"{x['type']} (isoform: {x['isoform_info']})" if pd.notna(x['isoform_info']) else x['type'], axis=1
    )
    
    # Drop the isoform_info column if you don't need it anymore
    ptm_df.drop(columns=['isoform_info'], inplace=True)
...
concat_ptm_df_filtered_reordered = concat_ptm_df_filtered.loc[:, ['Sequence S', 'Sequence T','Sequence P', 'Start position_P','End position_P',
                                                                  ...
                                                                  'LiP site','LiP Site Residue','Modification type','Modified Res #','Database']]
...
file_out_path = output_path + '\\lip_site_PTM_finder_results_Output.csv'
with open(file_out_path,'w',newline='') as filec:
        writerc = csv.writer(filec)
